...
describe('ImageSet', () => {
    it('#constructor(value)', () => {
        ...
        expect(Object.keys(imageSet.values).length).to.equal(2);
        expect(imageSet.values['1x'].toString()).to.equal(`url('cat.png')`);
        expect(imageSet.values['2x'].toString()).to.equal(`url('cat-2x.png')`);
    });
